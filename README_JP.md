...
FilterFusion を使用する前に、以下の要件を満たしていることを確認してください：

### 最小要件
- **Python**: 3.13 以上（ローカル開発では 3.14 も使用可）
...
**AdBlock ルール**：
```bash
python scripts/fetch_rules.py    # AdBlock ルールソースを取得
python scripts/merge_rules.py    # AdBlock ルールを統合・重複除去
```

**DNS フィルタリングルール**：
```bash
python scripts/fetch_dns_rules.py    # DNS ルールソースを取得
python scripts/merge_dns_rules.py    # DNS ルールを統合・重複除去
```

### **生成されたルールの使用**
- 生成されたルールファイルは `dist/` ディレクトリにあります
- カスタムルールに対応した広告ブロックツールに直接インポート
...
```
dist/adblock-main.txt              # AdBlock メインルール
dist/adblock-YYYYMMDD.txt          # AdBlock 日付アーカイブ
dist/dns-blocklist.txt             # DNS メインルール
dist/dns-blocklist-YYYYMMDD.txt    # DNS 日付アーカイブ
```
