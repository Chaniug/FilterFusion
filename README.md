...
编辑 `config/sources.txt` 文件，添加你需要聚合的 AdBlock 规则源列表：
...
编辑 `config/dns_sources.txt` 文件，添加你需要聚合的 DNS 过滤规则源列表：
...
- 删除无效和注释行
- 融合所有规则
- 输出标准格式：`dist/adblock-main.txt`（AdBlock）或 `dist/dns-blocklist.txt`（DNS）
...
**导入方法（以 AdGuard Home 为例）**：

1. 打开 AdGuard Home 管理界面
2. 进入「Filters」→「DNS blocklists」
3. 点击「Add a blocklist」
4. 粘贴 DNS 规则订阅链接
5. 点击「Add」添加
...
dist/adblock-YYYYMMDD.txt   # 按日期归档的规则文件
```

**DNS 过滤规则**：
```
dist/dns-blocklist.txt       # 主规则文件
dist/dns-blocklist-YYYYMMDD.txt   # 按日期归档的规则文件
```
