...
| 分类 | 匹配条件 | 示例 |
|------|----------|------|
| `exception` | 以 `@@` 开头 | `@@||example.com^$document` |
| `html_filter` | 含 `#%#` 或 `scriptlet(` 等 | `example.com#%#//scriptlet(...)` |
| `regex` | 以 `/` 开头和结尾的模式 | `/ads\.example\.com/` |
| `special` | 含 `$badfilter`, `$important`, `$csp=` | `||ads.com^$important` |
| `normal` | 普通域名/路径屏蔽规则 | `||ads.example.com^` |
| `comment` | 以 `!` 开头的注释行 | 仅保留前 10 条，不输出 |
...
| 注释 | 最高 | 以 `!` 开头 |
| 例外 | 高 | 以 `@@` 开头 |
| HTML过滤 | 中 | 包含 `#%#`、`scriptlet`、`+js(` 等 AdGuard 特有语法 |
| 正则 | 中 | 首尾均为 `/` 的正则表达式规则 |
| 特殊 | 低 | 包含 `$badfilter`、`$important`、`$csp=` 等修饰符 |
| 普通 | 最低 | 其余有效的屏蔽规则 |
